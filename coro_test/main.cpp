...
//	coro->child_coro_run(coroCreate);
	...
	/*
	工控编程测试程序
	上下左右方向键检测，按下后1000ms内弹起打印ok，否则打印timeout，模拟IO超时;
	空格键检测，按下后打印 _ ，500ms内没有弹起，每隔50ms打印 - ，模拟键盘字母输入;
	D,F键检测，10ms内两个键"同时"按下(必须"同时"，而不是一前一后)打印success，否则打印failure，模拟双手启动;
	鼠标按键检测，右键按下挂起动作，左键按下恢复动作，模拟暂停/恢复机构动作;
	生产者/消费者模型测试;
	ESC键退出，模拟急停操作;
