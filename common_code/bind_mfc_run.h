...
#define BIND_CORO_SEND()\
public:\
void send(boost_coro* coro, const boost::function<void ()>& h)\
{\
	coro->trig(boost::bind(&bind_mfc_run::send, (bind_mfc_run*)this, _1, h));\
}

#ifdef ENABLE_MFC_CORO
#define BIND_MFC_CORO(__dlg__, __base__)\
private:\
coro_handle create_mfc_coro(ios_proxy& ios, const boost_coro::main_func& mainFunc, size_t stackSize = DEFAULT_STACKSIZE)\
{\
	return boost_coro::create(mfc_strand::create(ios, this), mainFunc, stackSize);\
}\
\
coro_handle create_mfc_coro(const boost_coro::main_func& mainFunc, size_t stackSize = DEFAULT_STACKSIZE)\
{\
	return boost_coro::create(mfc_strand::create(this), mainFunc, stackSize);\
}\
\
