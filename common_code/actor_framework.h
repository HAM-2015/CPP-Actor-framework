...
private:
	...
	{
		std::shared_ptr<MsgPump_> res(new MsgPump_());
		res->_weakThis = res;
		res->_hasMsg = false;
		res->_waiting = false;
		res->_checkDis = false;
...
class MsgPool_
{
	...
	struct pump_handler
	{
		...
		bool try_pump(my_actor* host, dst_receiver& dst, unsigned char pumpID, bool& wait)
		{
			...
			return host->send<bool>(_thisPool->_strand, [&dst, &wait, pumpID, refThis_]()->bool
			{
				bool ok = false;
...
private:
	...
	{
		std::shared_ptr<MsgPool_> res(new MsgPool_(fixedSize));
		res->_weakThis = res;
		res->_strand = strand;
		res->_waiting = false;
		res->_closed = false;
