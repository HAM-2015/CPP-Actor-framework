...
template <typename Dispatcher, typename Handler, bool = false>
class wrapped_dispatch_front_handler
{
public:
	template <typename H>
	...
	template <typename D, typename H>
	wrapped_dispatch_front_handler(wrapped_dispatch_front_handler<D, H>&& sd)
		: _dispatcher(sd._dispatcher),
		_handler(std::move(sd._handler))
	{
	}
	...
	template <typename D, typename H>
	void operator=(wrapped_dispatch_front_handler<D, H>&& sd)
	{
		_dispatcher = sd._dispatcher;
		_handler = std::move(sd._handler);
	}
...
template <typename Dispatcher, typename Handler>
class wrapped_dispatch_front_handler<Dispatcher, Handler, true>
{
public:
	template <typename H>
	wrapped_dispatch_front_handler(Dispatcher* dispatcher, H&& handler)
	...
	template <typename D, typename H>
	wrapped_dispatch_front_handler(wrapped_dispatch_front_handler<D, H, true>&& sd)
		:_dispatcher(sd._dispatcher),
		_handler(std::move(sd._handler))
#if (_DEBUG || DEBUG)
		, _checkOnce(sd._checkOnce)
	...
	template <typename D, typename H>
	void operator=(wrapped_dispatch_front_handler<D, H, true>&& sd)
	{
		_dispatcher = sd._dispatcher;
		_handler = std::move(sd._handler);
#if (_DEBUG || DEBUG)
	...
	{
		assert(!_checkOnce->exchange(true));
		_dispatcher->dispatch_front(wrap_capture(FORCE_MOVE(_handler), FORCE_MOVE(args)...));
	}
