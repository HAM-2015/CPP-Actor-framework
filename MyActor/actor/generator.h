...
#define co_end_context(__ctx__) };\
	if (!co_self.__ctx){co_self.__ctx = new co_context_tag();\
...
#define co_end_context_init(__ctx__, __capture__, ...) _co_capture __capture__:__VA_ARGS__{}};\
	if (!co_self.__ctx){co_self.__ctx = new co_context_tag __capture__;\
	_co_end_context(__ctx__)
...
#define co_end_context_alloc(__alloc__, __ctx__) };\
	if (!co_self.__ctx){co_self.__ctx = new(__alloc__)co_context_tag();\
	_co_end_context(__ctx__)

#define co_end_context_alloc_init(__alloc__, __ctx__, __capture__, ...) _co_capture __capture__:__VA_ARGS__{}};\
	if (!co_self.__ctx){co_self.__ctx = new(__alloc__)co_context_tag __capture__;\
	_co_end_context(__ctx__)
...
#define co_call(...) do{\
	...
	DEBUG_OPERATION(co_self.__inside = false);\
	co_self.__coNext = -1;\
	co_self._co_push_stack((__COUNTER__+1)/2, _co_call(__VA_ARGS__));\
	return; case __COUNTER__/2:;\
	}while (0)

//在指定strand中，递归调用另一个generator，直到执行完毕后接着下一行
#define co_st_call(__strand__, ...) do{\
	_co_st_call(__strand__, co_async, __VA_ARGS__); _co_await;\
	}while (0)
...
#define co_chan_timed_io(__chan__, __ms__) co_await CoChanTimedIo_<decltype(__chan__)>(__ms__, __chan__, co_last_state, co_self, co_timer)
...
#define co_chan_timed_safe_io(__chan__, __ms__) co_await CoChanTimedSafeIo_<decltype(__chan__)>(__ms__, __chan__, co_last_state, co_self, co_timer)
...
#define co_csp_timed_send(__chan__, __ms__, __res__) co_await CoCspTimedSend_<decltype(__res__), decltype(__chan__)>(__ms__, __res__, __chan__, co_last_state, co_self, co_timer) <<
...
struct CoStCall_
{
	...
	static void call(SharedStrand&& strand, Ntf&& ntf, Handler&& handler, Args&&... args)
	{
		generator::create(std::forward<SharedStrand>(strand),
			std::bind(std::forward<Handler>(handler), __1, std::forward<Args>(args)...), std::forward<Ntf>(ntf))->run();
	}

	template <typename SharedStrand, typename Ntf, typename Handler>
	static void call(SharedStrand&& strand, Ntf&& ntf, Handler&& handler)
	{
		generator::create(std::forward<SharedStrand>(strand), std::forward<Handler>(handler), std::forward<Ntf>(ntf))->run();
	}
};

template <>
struct CoStCall_<true>
{
	template <typename SharedStrand, typename Ntf, typename Func, typename Obj, typename... Args>
	static void call(SharedStrand&& strand, Ntf&& ntf, Func func, Obj&& obj, Args&&... args)
	{
		generator::create(std::forward<SharedStrand>(strand),
			std::bind(func, std::forward<Obj>(obj), __1, std::forward<Args>(args)...), std::forward<Ntf>(ntf))->run();
	}
};

template <typename SharedStrand, typename Ntf, typename Handler, typename Unknown, typename... Args>
static void _co_st_call(SharedStrand&& strand, Ntf&& ntf, Handler&& handler, Unknown&& unkown, Args&&... args)
{
	CoStCall_<CheckClassFunc_<RM_REF(Handler)>::value>::call(std::forward<SharedStrand>(strand), std::forward<Ntf>(ntf),
		std::forward<Handler>(handler), std::forward<Unknown>(unkown), std::forward<Args>(args)...);
}

template <typename SharedStrand, typename Ntf, typename Handler>
static void _co_st_call(SharedStrand&& strand, Ntf&& ntf, Handler&& handler)
{
	static_assert(!CheckClassFunc_<RM_REF(Handler)>::value, "");
	CoStCall_<false>::call(std::forward<SharedStrand>(strand), std::forward<Ntf>(ntf), std::forward<Handler>(handler));
}
...
struct CoCall_
{
	...
	static std::function<void(generator&)> call(Handler&& handler, Args&&... args)
	{
		return std::bind(std::forward<Handler>(handler), __1, std::forward<Args>(args)...);
	}

	template <typename Handler>
	static std::function<void(generator&)> call(Handler&& handler)
	{
...
struct CoCall_<true>
{
	...
	static std::function<void(generator&)> call(Func func, Obj&& obj, Args&&... args)
	{
...
static std::function<void(generator&)> _co_call(Handler&& handler, Unknown&& unkown, Args&&... args)
{
	return CoCall_<CheckClassFunc_<RM_REF(Handler)>::value>::call(std::forward<Handler>(handler), std::forward<Unknown>(unkown), std::forward<Args>(args)...);
}

template <typename Handler>
static std::function<void(generator&)> _co_call(Handler&& handler)
{
	static_assert(!CheckClassFunc_<RM_REF(Handler)>::value, "");
	return CoCall_<false>::call(std::forward<Handler>(handler));
}
//////////////////////////////////////////////////////////////////////////
...
{
	CoCspTimedSend_(int ms, R& res, CspChan& chan, co_async_state& state, generator& host, overlap_timer::timer_handle& timer) :_tm(ms), _res(res), _chan(chan), _state(state), _host(host), _timer(timer) {}
	template <typename Arg> void operator<<(Arg&& arg) { push(this, std::forward<Arg>(arg)); }
	...
	void operator<<(void_type&&) { push(this); }
private:
	template <typename... Args> static void push(CoCspTimedSend_* this_, Args&&... args) { co_generator = this_->_host; this_->_chan.timed_push(this_->_timer, this_->_tm, co_async_result_(this_->_state, this_->_res), std::forward<Args>(args)...); }
private:
...
public:
	...
	void timed_pop(overlap_timer::timer_handle& timer, int ms, Notify&& ntf)
	{
		if (_strand->running_in_this_thread())
		{
			_timed_pop(timer, std::forward<Notify>(ntf), ms);
		}
		else
		{
			typedef RM_CREF(Notify) Notify_;
			_strand->post(std::bind([this, ms, &timer](Notify_& ntf)
			{
				_timed_pop(timer, std::move(ntf), ms);
			}, std::forward<Notify>(ntf)));
		}
	}
...
private:
	...
	template <typename Notify>
	void _timed_pop(overlap_timer::timer_handle& timer, Notify&& ntf, int ms)
	{
		assert(_strand->running_in_this_thread());
		if (_closed)
		{
...
public:
	...
	void timed_push(overlap_timer::timer_handle& timer, int ms, Notify&& ntf, Args&&... msg)
	{
		if (_strand->running_in_this_thread())
		{
			_timed_push(timer, msg_type(std::forward<Args>(msg)...), std::forward<Notify>(ntf), ms);
		}
		else
		{
			typedef RM_CREF(Notify) Notify_;
			_strand->post(std::bind([this, ms, &timer](msg_type& msg, Notify_& ntf)
			{
				_timed_push(timer, std::move(msg), std::move(ntf), ms);
			}, msg_type(std::forward<Args>(msg)...), std::forward<Notify>(ntf)));
		}
	}
...
private:
	...
	template <typename Notify>
	void _timed_push(overlap_timer::timer_handle& timer, msg_type&& msg, Notify&& ntf, int ms)
	{
		assert(_strand->running_in_this_thread());
		if (_closed)
		{
...
public:
	...
	void timed_lock(overlap_timer::timer_handle& timer, long long id, int ms, Notify&& ntf)
	{
		...
		{
			typedef RM_CREF(Notify) Notify_;
			_strand->post(std::bind([this, id, ms, &timer](Notify_& ntf)
			{
				_timed_lock(timer, id, ms, std::move(ntf));
			}, std::forward<Notify>(ntf)));
		}
	}
	...
	void timed_lock(overlap_timer::timer_handle& timer, long long id, int ms, Notify&& ntf, LockedNtf&& lockedNtf)
	{
		...
		else
		{
			typedef RM_CREF(Notify) Notify_;
			typedef RM_CREF(LockedNtf) LockedNtf_;
			_strand->post(std::bind([this, id, ms, &timer](Notify_& ntf, LockedNtf_& lockedNtf)
			{
				_timed_lock(timer, id, ms, std::move(ntf), std::move(lockedNtf));
			}, std::forward<Notify>(ntf), std::forward<LockedNtf>(lockedNtf)));
		}
	}
...
public:
	...
	void timed_lock(overlap_timer::timer_handle& timer, long long id, int ms, Notify&& ntf)
	{
		...
		{
			typedef RM_CREF(Notify) Notify_;
			self_strand()->post(std::bind([this, id, ms, &timer](Notify_& ntf)
			{
				_timed_lock(timer, id, ms, std::move(ntf));
			}, std::forward<Notify>(ntf)));
		}
	...
	void timed_lock(overlap_timer::timer_handle& timer, long long id, int ms, Notify&& ntf, LockedNtf&& lockedNtf)
	{
		...
		else
		{
			typedef RM_CREF(Notify) Notify_;
			typedef RM_CREF(LockedNtf) LockedNtf_;
			self_strand()->post(std::bind([this, id, ms, &timer](Notify_& ntf, LockedNtf_& lockedNtf)
			{
				_timed_lock(timer, id, ms, std::move(ntf), std::move(lockedNtf));
			}, std::forward<Notify>(ntf), std::forward<LockedNtf>(lockedNtf)));
		}
	...
	void timed_lock_shared(overlap_timer::timer_handle& timer, long long id, int ms, Notify&& ntf)
	{
		...
		else
		{
			...
			self_strand()->post(std::bind([this, id, ms, &timer](Notify_& ntf)
			{
				_timed_lock_shared(timer, id, ms, std::move(ntf));
			}, std::forward<Notify>(ntf)));
		}
	}
	...
	void timed_lock_shared(overlap_timer::timer_handle& timer, long long id, int ms, Notify&& ntf, LockedNtf&& lockedNtf)
	{
		...
		else
		{
			...
			self_strand()->post(std::bind([this, id, ms, &timer](Notify_& ntf, LockedNtf_& lockedNtf)
			{
				_timed_lock_shared(timer, id, ms, std::move(ntf), std::move(lockedNtf));
			}, std::forward<Notify>(ntf), std::forward<LockedNtf>(lockedNtf)));
		}
	}
