...
void actor_test(my_actor* self)
{
	...
	{//模拟消息转发，转发切换
		...
		{//消费者
			...
			{
				auto conCmh = self->connect_msg_pump<move_test>();
				move_test msg = self->pump_msg(conCmh);
			...
			while (true)
			{
				self->msg_agent_to<move_test>(agent1);//消息由agent1代理
				self->sleep(5000);
				self->msg_agent_to<move_test>(agent2);//断开agent1代理，切换到agent2代理
				self->sleep(2000);
				auto conCmh = self->connect_msg_pump<move_test>();//停止消息代理，由自己处理
				for (int i = 0; i < 3; i++)
				{
					move_test msg = self->pump_msg(conCmh);
					printf("数据:%d 发送者:%d 接收者:%d\n", msg._count->_id / 10000, msg._count->_id % 10000, (int)self->self_id());
		...
		auto writer = self->connect_msg_notifer_to<move_test>(actorConsumer);//连接消息到actorConsumer
		auto test_producer = [writer](my_actor* self)
		{//生产者
			for (int i = 0; true; i++)
			{
				writer(move_test(i*10000 + (int)self->self_id()));
				self->sleep(2000);
			}
		};
	...
	{//模拟同步消息发送，CSP模型消息发送
		syncPush = self->create_child_actor(self->self_strand(), [&](my_actor* self)
		{
			try
			{
				...
				while (true)
				{
					...
					move_test r = cspMsg.invoke(self, move_test(i++), i);
					printf("csp return %d\n", r._count->_id);
				}
			}
		...
		auto h = [&](my_actor* self)
		{
			try
			{
				...
				while (true)
				{
					move_test id = syncMsg.take(self);
					printf("sync %d %d\n", id._count->_id, (int)self->self_id());
					cspMsg.wait_invoke(self, [&](const move_test& id, int i)->move_test
					{
						printf("csp %d\n", id._count->_id);
						self->sleep(1000);
						return move_test(id._count->_id * 10000);
					});
				}
			}
			catch (sync_csp_close_exception)
