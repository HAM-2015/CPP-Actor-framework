...
std::shared_ptr<text_stream_io> text_stream_io::create( shared_strand strand, std::shared_ptr<stream_io_base> ioObj, const std::function<void (shared_data)>& h )
{
	...
	res->_writerPipeIn = wc->make_msg_notifer_to_self(res->_writerPipeOut);
...
void text_stream_io::readActor( my_actor* self )
{
	...
	while (true)
	{
		...
		size_t length;
		_ioObj->async_read_some(buff + msgLength, sizeof(buff)-msgLength, self->make_asio_context(ec, length));
		if (ec || 0 == length)
		{
...
void text_stream_io::writeActor( my_actor* self )
{
	...
	while (true)
	{
		...
		}
		{
			boost::system::error_code ec;
			size_t length;
			_ioObj->async_write((unsigned char*)msg->data(), msg->size(), self->make_asio_context(ec, length));
			if (ec)
			{
				break;
			}
		}
		{
			boost::system::error_code ec;
			size_t length;
			_ioObj->async_write((unsigned char*)textTail.c_str(), textTail.size(), self->make_asio_context(ec, length));
			if (ec)
			{
				break;
			}
